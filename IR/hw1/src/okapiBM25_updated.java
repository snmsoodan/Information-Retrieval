...
//						System.out.println(i);
						...
						for(int i1=0;i1<hitsArray.size();i1++)
						{
							JSONObject objects = (JSONObject)hitsArray.get(i1);
							Double tfscore1=(Double)objects.get("_score");
							if(tfscore1>0.0)
							{
								count1++;
							}
...
//									tfIDf= (tfScore/(tfScore+1.5+(0.5*(documentLength/totallen))))*Math.log(84678/count1);
									Double term1 = (84678+0.5)/(count1+0.5);
								    Double term2 = (tfScore+(1.2*tfScore))/(tfScore+(1.2*(0.25+(0.75*(documentLength/247.0)))));
								    Double term3 = (double) ((tfScore+(500*tfScore))/(tfScore+500));
								    BM25= Math.log(term1*term2*term3);
								    if(RankedList.containsKey(documentid))
													{
														RankedList.put(documentid, RankedList.get(documentid)+BM25);
													}
													else
													{ 
														
														RankedList.put((String)((JSONObject)objects).get("_id"), BM25);
													}
								}
