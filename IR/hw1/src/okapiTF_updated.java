...
//		for (String line : Files.readAllLines(Paths.get("C:\\Users\\Snm\\Downloads\\AP_DATA\\query_desc.51-100.short.txt")))
		for (String line : Files.readAllLines(Paths.get("C:/Users/Snm/Downloads/AP_DATA/qu1.txt")))
		{
...
//								FunctionScoreQueryBuilder(QueryBuilders.termQuery("text", i)
//								,new ScriptScoreFunctionBuilder(new Script(org.elasticsearch.script.ScriptType.INLINE,"getTF","groovy", params)));
										
//						QueryBuilder qb=scriptQuery(new Script(ScriptType.INLINE,"groovy","_index[field][term].tf()",params));
...
//							for (SearchHit objects : response.getHits().getHits()) {
//System.out.println(objects);
						...
						for(int j=0;j<hitsArray.size();j++)
						{
							...
							JSONObject fields=(JSONObject) ((JSONObject)objects).get("fields");

							int tfScore=Integer.parseInt(((JSONArray)fields.get("field")).get(0).toString());
							if(tfScore>0.0)
								{
									Double okapitf=0.00;
									...
									{	
//										System.out.println("term freq  "+(Integer)((Map) terms.get(map1.getKey())).get("term_freq"));
								    ...
								    if(RankedList.containsKey(documentid))
													{
														RankedList.put(documentid, RankedList.get(documentid)+okapitf);
													}
													else
													{ 
														
														RankedList.put((String)((JSONObject)objects).get("_id"), okapitf);
													}
								}
